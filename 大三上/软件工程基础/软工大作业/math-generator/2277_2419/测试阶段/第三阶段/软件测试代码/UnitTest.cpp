...
namespace UnitTest
{
	...
	public:
		...
		TEST_METHOD(testCalculater)
		{
			...
			calculater.power(Number(1, 2), Number(-2, 1));
		}
		...
		TEST_METHOD(testFile)
		{
			...
			file->setProblemPath("problem.txt");
			file->setRecordPath("reocrd.txt");

			file->removeFile(file->getProblemPath());
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				file->writeToFile("right1, wrong2");
			}
